                                   ...
                                   IComparer
    {
        ...
        public virtual NaturalSortOrder SortOrder
        {
            get
            {
                switch ( _order )
                {
                    case 1:  return NaturalSortOrder.Ascending;
                    case -1: return NaturalSortOrder.Descending;
                    default: return NaturalSortOrder.Ascending;
                }
            }
            set
            {
        ...
        protected virtual int LocalCompare( string s1, string s2 )
        {
            ...
            var filter = ( CharTypes )( _options & NaturalComparerOptions.NumberAll );
            var t1     = CharTypes.None;
            var t2     = CharTypes.None;
            var p1     = 0;
            var p2     = 0;
            var c1     = char.MinValue;
            ...
            while ( p1 < s1.Length && p2 < s2.Length )
            {
                ...
                if ( ( IgnoreNumber || ( IgnoreNumber == false && t1 == t2 ) ) && t1 != CharTypes.None && t2 != CharTypes.None )
                {
                    ...
                    var success = GetNumber( s1, t1, ref i1, out v1 ) && GetNumber( s2, t2, ref i2, out v2 );
                    if ( success )
                    {
                        if ( v1 < v2 )
                        {
                    ...
                    {
                        var diff = CompareChar( s1[ p1 ], s2[ p2 ] );
                        if ( diff != 0 )
                        {
                            return diff;
                        }
        ...
        private int CompareChar( char c1, char c2 )
        {
            // 前中後、上中下の整列を考慮する
            var list = "上前中下後";
            var p1   = list.IndexOf( c1 );
            var p2   = list.IndexOf( c2 );
        ...
        private class JpRomanNumberConverter : INumberComverter
        {
            ...
            private readonly bool _isMultiChar;

            /// <summary>ローマ数字の 1 を表す char。</summary>
            private readonly char _romanOne;
